...
console.log(getFruitColor (fruits[4]))

function getFruitid (fruit ){
    return fruit.id 
} 
console.log(getFruitid (fruits[1]))
...
function matchesdescription(fruit,taste){
    return fruit.taste == taste
} 
console.log(matchesdescription(fruits[2],"Citrusy"))
...
//   unfigured way!!
//     let sweettastefriuts = 0;
//     fruits.foreach( (fruits)=>{if (fruits.taste == `sweet` ){sweettastefriuts++}
//     } ) 
//     }
//     return sweettastefriuts;

function countSweetFruits(fruits,taste) {
    return fruits.filter( (x) => x.taste == taste).length;

  }
  console.log(countSweetFruits(fruits, `Tangy`));
